...
st.set_page_config(page_title='Informações Gerais', page_icon=':)', layout = 'wide')
...
def map_country(df):
   ...
   folium_static(map, width=1300, height=500) #para usar no streamlit
...
st.header('Zomato Restaurants Apps  - Onde você encontra o seu próximo restaurante favorito, e o melhor, em qualquer parte do mundo')

image_path = 'zomato.png'
image=Image.open('zomato.png')
st.sidebar.image( image,width=300)

st.sidebar.markdown('#### Métrica Global do Zomato Restaurant Apps')
st.sidebar.markdown("""   """)
...
tab1, tab2 = st.tabs (['Métricas Globais' , '---'])  
...
#Maps das localizações
    ...
    with st.container(): 
         st.markdown('##### Localização dos Restaurantes cadastrados na plataforma Zomato App')
         map_country(df)
         st.markdown('###### **Localização dos Restaurantes por País, Cidade, Tipo de Culinária e moeda corrente')
