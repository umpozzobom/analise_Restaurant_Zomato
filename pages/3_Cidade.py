...
##          FUNÇÕES DO CODIGO                    #
#=========================================================#
def number_restaurant_city(df):
             df_aux = (df.loc[ :, [ 'City', 'Restaurant ID']].groupby(['City']).count().reset_index()
                         .sort_values(by = 'Restaurant ID', ascending = False).head(10))
             fig = px.bar(df_aux, x= 'City', y= 'Restaurant ID', color = 'City', title='Cidades com maior número de restaurantes registrados', color_continuous_scale = px.colors.sequential.gray, height=450,width = 700 )
             return fig
...
st.subheader('Analise das Cidades cadastradas na Zomato Restaurant Apps')
...
with tab1:
     with st.container():
          fig = number_restaurant_city(df)
          st.plotly_chart(fig, use_container_width=True)

     
     with st.container():
           st.markdown('##### Top 10 cidades cujos restaurantes realizam reserva de mesa, pedidos online e delivery') 
     ...
     with st.container():   
                df_aux = df.loc[:, ['Country', 'City', 'Cuisines']].groupby(['Country', 'City']).nunique().sort_values(by = 'Cuisines', ascending= False).head(15).reset_index()
                fig = px.line(df_aux,  x= 'City', y = 'Cuisines', title = 'Cidades com mais tipos de culinárias distintas',color_discrete_sequence=px.colors.sequential.RdBu)
                st.plotly_chart(fig, use_container_width = True)
     ...
     with st.container():
        ...
        with col1:
             st.markdown('**Restaurantes com avaliação média menor que 2.5**')
        ...
        with col2:
             st.markdown('**Restaurantes com avaliação média maior que 4.0**')
